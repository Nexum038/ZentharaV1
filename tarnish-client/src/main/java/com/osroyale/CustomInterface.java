...
//        personalStores(textDrawingAreas);
        ...
        emote();
        testInterface(textDrawingAreas);
        ...
        perkInterface(textDrawingAreas);
        skillingPerkInterface(textDrawingAreas);
    }
...
//        bank.child(60, interfaceId + 78, 19, 240);
    ...
    // COMBAT PERKS INTERFACE  (id 60200)
    // =========================================================
    private static void perkInterface(TextDrawingArea[] tda) {

        final int W = 490;
        final int H = 320;

        RSInterface rsi = addInterface(60200);
        rsi.width  = W;
        rsi.height = H;

        addRectangle(60201, 0, 0x3D2B1F, false, W,     H);
        addRectangle(60202, 0, 0x2B1F15, true,  W - 2, H - 2);
        addText(60203, "Combat Perks", tda, 2, 0xFF981F, true, true);
        addRectangle(60204, 0, 0xFF981F, true, W - 20, 1);

        addHoverButton(60205, 24, 16, 16, "Close", 250, 60206, 3);
        addHoveredButton(60206, 25, 16, 16, 60207);

        addText(60208, "Combat Points:", tda, 1, 0xFF981F, false, true);
        addText(60209, "0",              tda, 0, 0xFFFFFF, false, true);
        addText(60210, "Next XP cost:",  tda, 1, 0xFF981F, false, true);
        addText(60211, "10,000 XP",      tda, 0, 0xFFFFFF, false, true);
        addText(60212, "Total earned:",  tda, 1, 0xFF981F, false, true);
        addText(60213, "0",              tda, 0, 0xFFFFFF, false, true);

        addRectangle(60214, 0, 0x5A3D28, true, 1, 68);

        // Tabs — using staticons.dat sprites: 197=attack(sword), 209=mining(pickaxe), 208=smithing(hammer)
        addRectangle(58800, 0, 0xFF981F, false, 34, 34);                         // active highlight box (combat)
        addHoverButton(58801, -1, 30, 30, "Skilling Perks", -1, 58802, 1);     // signed: -6735
        addSprite(58802, 209);                                                    // Skilling sprite (mining pickaxe)
        addSprite(58803, 208);                                                    // Utility sprite (smithing)
        addSprite(58804, 197);                                                    // Combat sprite (attack sword)

        addRectangle(60230, 0, 0x5A3D28, true, W - 20, 1);
        addText(60231, "Perk",   tda, 0, 0xFFB83F, false, true);
        addText(60232, "Level",  tda, 0, 0xFFB83F, false, true);
        addText(60233, "Action", tda, 0, 0xFFB83F, false, true);
        addText(60234, "Status", tda, 0, 0xFFB83F, false, true);

        RSInterface scroll = addTabInterface(60250);
        scroll.scrollPosition = 0;
        scroll.contentType    = 0;
        scroll.width          = W - 30;
        scroll.height         = 195;
        scroll.scrollMax      = 288;
        ...
        for (int i = 0; i < 8; i++) {
            int base  = 60251 + (i * 7);
            int rowY  = i * 36;
            int rowBg = (i % 2 == 0) ? 0x3A2920 : 0x2F2118;

            addRectangle(base, 0, rowBg, true, W - 30, 34);
            addText(base + 1, "", tda, 1, 0xFF981F, false, true);
            addText(base + 2, "", tda, 0, 0xFFB83F, true,  true);

            addHoverButton(base + 3, 247, 100, 28, "Unlock / Upgrade", -1, base + 4, 1);
            addText(base + 4, "Unlock", tda, 1, 0xFF981F, true, true);
            interfaceCache[base + 4].textHoverColor = 0xFFFFFF;

            addHoverButton(base + 5, 247, 100, 28, "Activate / Deactivate", -1, base + 6, 1);
            addText(base + 6, "--", tda, 1, 0xFF981F, true, true);
            interfaceCache[base + 6].textHoverColor = 0xFFFFFF;

            scroll.child(sc++, base,     0,   rowY);
            scroll.child(sc++, base + 1, 8,   rowY + 9);
            scroll.child(sc++, base + 2, 175, rowY + 9);
            scroll.child(sc++, base + 3, 215, rowY + 3);
            scroll.child(sc++, base + 4, 265, rowY + 10);
            scroll.child(sc++, base + 5, 340, rowY + 3);
            scroll.child(sc++, base + 6, 390, rowY + 10);
        }

        rsi.totalChildren(24);
        rsi.child(0,  60201, 0,   0);
        rsi.child(1,  60202, 1,   1);
        rsi.child(2,  60203, 245, 6);
        rsi.child(3,  60204, 10,  27);
        rsi.child(4,  60205, 469, 8);
        rsi.child(5,  60206, 469, 8);
        rsi.child(6,  60208, 14,  44);
        rsi.child(7,  60209, 140, 44);
        rsi.child(8,  60210, 14,  60);
        rsi.child(9,  60211, 140, 60);
        rsi.child(10, 60212, 14,  76);
        rsi.child(11, 60213, 140, 76);
        rsi.child(12, 60214, 255, 40);
        rsi.child(13, 58800, 258, 40);   // Combat active highlight box
        rsi.child(14, 58801, 300, 40);   // Skilling tab button (invisible)
        rsi.child(15, 58802, 300, 40);   // Skilling sprite
        rsi.child(16, 58803, 342, 40);   // Utility sprite
        rsi.child(17, 58804, 260, 42);   // Combat sprite (on top)
        rsi.child(18, 60230, 10,  107);
        rsi.child(19, 60231, 20,  111);
        rsi.child(20, 60232, 170, 111);
        rsi.child(21, 60233, 230, 111);
        rsi.child(22, 60234, 355, 111);
        rsi.child(23, 60250, 15,  122);
    }

    // =========================================================
    // SKILLING PERKS INTERFACE  (id 59700)
    // =========================================================
    private static void skillingPerkInterface(TextDrawingArea[] tda) {

        final int W = 490;
        final int H = 320;

        RSInterface rsi = addInterface(59700);
        rsi.width  = W;
        rsi.height = H;

        addRectangle(59701, 0, 0x3D2B1F, false, W,     H);
        addRectangle(59702, 0, 0x2B1F15, true,  W - 2, H - 2);
        addText(59703, "Skilling Perks", tda, 2, 0xFF981F, true, true);
        addRectangle(59704, 0, 0xFF981F, true, W - 20, 1);

        addHoverButton(59705, 24, 16, 16, "Close", 250, 59706, 3);
        addHoveredButton(59706, 25, 16, 16, 59707);

        addText(59708, "Skilling Points:", tda, 1, 0xFF981F, false, true);
        addText(59709, "0",                tda, 0, 0xFFFFFF, false, true);
        addText(59710, "Next XP cost:",    tda, 1, 0xFF981F, false, true);
        addText(59711, "10,000 XP",        tda, 0, 0xFFFFFF, false, true);
        addText(59712, "Total earned:",    tda, 1, 0xFF981F, false, true);
        addText(59713, "0",                tda, 0, 0xFFFFFF, false, true);

        addRectangle(59714, 0, 0x5A3D28, true, 1, 68);

        // Tabs — IDENTICAL structure to working version, only colors/text changed
        // Tabs — using staticons.dat sprites: 197=attack(sword), 209=mining(pickaxe), 208=smithing(hammer)
        addHoverButton(58900, -1, 30, 30, "Combat Perks", -1, 58901, 1);       // signed: -6636
        addSprite(58901, 197);                                                    // Combat sprite (attack sword)
        addRectangle(58902, 0, 0xFF981F, false, 34, 34);                         // active highlight box (skilling)
        addSprite(58903, 208);                                                    // Utility sprite (smithing)
        addSprite(58904, 209);                                                    // Skilling sprite (mining pickaxe)

        addRectangle(59730, 0, 0x5A3D28, true, W - 20, 1);
        addText(59731, "Perk",   tda, 0, 0xFFB83F, false, true);
        addText(59732, "Level",  tda, 0, 0xFFB83F, false, true);
        addText(59733, "Action", tda, 0, 0xFFB83F, false, true);
        addText(59734, "Status", tda, 0, 0xFFB83F, false, true);

        RSInterface scroll = addTabInterface(59750);
        scroll.scrollPosition = 0;
        scroll.contentType    = 0;
        scroll.width          = W - 30;
        scroll.height         = 195;
        ...
        rsi.totalChildren(24);
        rsi.child(0,  59701, 0,   0);
        rsi.child(1,  59702, 1,   1);
        rsi.child(2,  59703, 245, 6);
        rsi.child(3,  59704, 10,  27);
        rsi.child(4,  59705, 469, 8);
        rsi.child(5,  59706, 469, 8);
        rsi.child(6,  59708, 14,  44);
        rsi.child(7,  59709, 140, 44);
        rsi.child(8,  59710, 14,  60);
        rsi.child(9,  59711, 140, 60);
        rsi.child(10, 59712, 14,  76);
        rsi.child(11, 59713, 140, 76);
        rsi.child(12, 59714, 255, 40);
        rsi.child(13, 58900, 258, 40);   // Combat tab button (invisible)
        rsi.child(14, 58901, 258, 40);   // Combat sprite
        rsi.child(15, 58902, 300, 40);   // Skilling active highlight box
        rsi.child(16, 58903, 342, 40);   // Utility sprite
        rsi.child(17, 58904, 302, 42);   // Skilling sprite (on top)
        rsi.child(18, 59730, 10,  107);
        rsi.child(19, 59731, 20,  111);
        rsi.child(20, 59732, 170, 111);
        rsi.child(21, 59733, 230, 111);
        rsi.child(22, 59734, 355, 111);
        rsi.child(23, 59750, 15,  122);
    }

    private static void testInterface(TextDrawingArea[] tda) {
        RSInterface rsi = interfaceCache[24126];
        rsi.width = 500;
        rsi.height = 320;
    }
