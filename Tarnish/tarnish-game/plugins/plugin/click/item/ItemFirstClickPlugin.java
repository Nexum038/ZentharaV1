    ...
    protected boolean firstClickItem(Player player, ItemClickEvent event) {
        switch (event.getItem().getId()) {
         ...
         /*   case 80:
            ...
            case 6543: {
                if (player.dropRateBoost >= 5) {
                    player.send(new SendMessage("You have already used this item and received the drop rate boost!"));
                    return true;
                }
                player.inventory.remove(6543, 1);
                player.dropRateBoost += 5;
                double totalBonus = PlayerRight.getDropRateBonus(player);
                player.send(new SendMessage("<col=158A76>You have unlocked a permanent +5% drop rate boost!"));
                player.send(new SendMessage("<col=158A76>Your total drop rate bonus is now: " + totalBonus + "%."));
                break;
            }
