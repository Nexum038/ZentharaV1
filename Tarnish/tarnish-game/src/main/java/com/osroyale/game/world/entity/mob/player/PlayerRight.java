...
public enum PlayerRight {
    ...
    public static int dropRateIncrease(Player player, int roll) {
        double bonus = getDropRateBonus(player);
        return (int) (roll * (1.0 - bonus / 100.0));
    }

    public static double getDropRateBonus(Player player) {
        double base;
        if (isKing(player)) base = 16;
        else if (isElite(player)) base = 13;
        else if (isExtreme(player)) base = 10;
        else if (isSuper(player)) base = 7;
        else if (isDonator(player)) base = 5;
        else base = 0;

        // Item boost
        base += player.dropRateBoost;

        // Pet boost (only when pet is following)
        if (player.pet != null) {
            double petBonus = PetData.forNpc(player.pet.id)
                    .map(PetData::getDropRateBonus)
                    .orElse(0.0);
            base += petBonus;
        }
