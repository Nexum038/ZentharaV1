...
public class DropDisplay {
    ...
    public static void display(Player player, NpcDropTable definition) {
        ...
        double drOffset = 1 + (PlayerRight.getDropRateBonus(player) / 100.0);
...
//            player.send(new SendString(!valid ? "" : item.getName() + " (<col=ffffff>" + item.getId() + "</col>)", string));
            ...
            string++;
            player.send(new SendString(!valid ? "" : getDropRateString(drop, drOffset), string));
            string++;
        }
    }

    private static String getDropRateString(NpcDrop drop, double drOffset) {
        if (drop.type == NpcDropChance.ALWAYS) {
            return "Always";
        }
        int weight = drop.chance > 0 ? drop.chance : drop.type.weight;
        int boostedWeight = (int) Math.max(1, Math.round(weight / drOffset));
        if (boostedWeight == weight) {
            return "1/" + weight;
        }
        return "1/" + weight + " <col=158A76>(1/" + boostedWeight + ")</col>";
    }
}