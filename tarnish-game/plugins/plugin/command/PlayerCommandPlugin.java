...
public class PlayerCommandPlugin extends CommandExtension {
    ...
    protected void register() {
        ...
        commands.add(new Command("dr", "droprate") {
            @Override
            public void execute(Player player, CommandParser parser) {
                double petBonus = player.pet != null ? PetData.forNpc(player.pet.id).map(PetData::getDropRateBonus).orElse(0.0) : 0.0;
                double rankBonus = PlayerRight.getDropRateBonus(player) - player.dropRateBoost - petBonus;
                double itemBonus = player.dropRateBoost;
                double totalBonus = PlayerRight.getDropRateBonus(player);
                player.send(new SendMessage("<col=ffffff>--- Drop Rate Bonus ---"));
                player.send(new SendMessage("Rank (<col=ff9933>" + player.right.getName() + "</col>): <col=158A76>+" + rankBonus + "%"));
                if (itemBonus > 0) {
                    player.send(new SendMessage("Item boosts: <col=158A76>+" + itemBonus + "%"));
                }
                if (petBonus > 0) {
                    player.send(new SendMessage("Pet (<col=ff9933>" + player.pet.getName() + "</col>): <col=158A76>+" + petBonus + "%"));
                }
                player.send(new SendMessage("Total: <col=158A76>" + totalBonus + "%"));
            }
        });
    }
