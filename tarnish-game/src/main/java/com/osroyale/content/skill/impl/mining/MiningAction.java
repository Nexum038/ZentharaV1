...
public class MiningAction extends Action<Player> {
    ...
    MiningAction(Player player, GameObject object, OreData ore, PickaxeData pickaxe) {
        super(player, getMiningDelay(player), false);
        this.object = object;
        this.ore = ore;
        this.pickaxe = pickaxe;
    }

    private static int getMiningDelay(Player player) {
        int delay = 3;
        int reduction = player.perk.getSkillingTickReduction(SkillingPerk.SWIFT_HANDS);
        return Math.max(1, delay - reduction);
    }
