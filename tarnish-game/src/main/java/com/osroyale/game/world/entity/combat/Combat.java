...
public class Combat<T extends Mob> {
    ...
    public void submitStrategy(Mob defender, CombatStrategy<? super T> strategy) {
        ...
        // Swift Strikes: reduce melee attack cooldown based on active perk level
        if (attacker.isPlayer() && (fightStyle == FightStyle.ACCURATE || fightStyle == FightStyle.AGGRESSIVE
                || fightStyle == FightStyle.DEFENSIVE || fightStyle == FightStyle.CONTROLLED)) {
            int reduction = attacker.getPlayer().perk.getMeleeTickReduction();
            if (reduction > 0) {
                cooldown = Math.max(1, cooldown - reduction);
            }
        }
